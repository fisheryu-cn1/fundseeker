#!/usr/bin/env python3
"""Run a fund-company collector (YFD, HTF, ...) and persist results.
...
from fundseeker.runner import FUND_COLLECTORS, run_fund_company


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Collect fund company product list and NAV."
    )
    parser.add_argument(
        "code",
        choices=list(FUND_COLLECTORS.keys()),
        help="Fund company institution code",
    )
    args = parser.parse_args()

    result = run_fund_company(args.code)
    print(
