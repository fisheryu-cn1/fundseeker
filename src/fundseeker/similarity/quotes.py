...
_WORKER_CLIENT: threading.local = threading.local()


def _get_worker_client() -> PoliteHttpClient:
    """Return one PoliteHttpClient per worker thread.

    PoliteHttpClient keeps mutable state (last request time, robots cache) on a
    requests.Session, so it is not thread-safe.  Sharing one instance across
    threads would race on the polite-delay bookkeeping; instead each worker
    thread lazily creates its own client.
    """
    client = getattr(_WORKER_CLIENT, "client", None)
    if client is None:
        client = PoliteHttpClient(min_delay=0.1, max_delay=0.3, respect_robots_txt=False)
        _WORKER_CLIENT.client = client
    return client


def _fetch_worker(
    args: tuple[str, str, str | None, date, date],
) -> tuple[list[dict[str, Any]], tuple[str, str] | None]:
    """Fetch quotes for a single symbol inside a worker thread."""
    code, market, name, start_date, end_date = args
...
def refresh_stock_quotes(
    ...
    env_workers = os.environ.get("FUNDSEEKER_QUOTE_WORKERS")
    ...
    tasks = [(code, market, name, start_date, end_date) for code, market, name in symbols]
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(_fetch_worker, task) for task in tasks]
        # as_completed() — yield each result as soon as it finishes, instead of
        # blocking on executor.map()'s order-preserving iterator (which would
        # wait for the slowest symbol to flush the whole queue).
        for future in as_completed(futures):
            quotes, failure = future.result()
            if failure:
